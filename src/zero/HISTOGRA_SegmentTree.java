...
public class HISTOGRA_SegmentTree {
...
// A Divide and Conquer Program to find maximum rectangular area in a histogram
    ...
    static int minVal(int[] hist, int i, int j) {
        if (i == -1) return j;
        if (j == -1) return i;
    ...
    static int RMQUtil(int[] hist, int[] st, int ss, int se, int qs, int qe, int index) {
...
// qe (query end).  It mainly uses RMQUtil()
    static int RMQ(int[] hist, int[] st, int n, int qs, int qe) {
...
// si is index of current node in segment tree st
    static int constructSTUtil(int hist[], int ss, int se, int[] st, int si) {
    ...
    static int[] constructST(int hist[], int n) {
...
// It uses segment tree 'st' to find the minimum value in hist[l..r]
    static int getMaxAreaRec(int[] hist, int[] st, int n, int l, int r) {
        // Base cases
